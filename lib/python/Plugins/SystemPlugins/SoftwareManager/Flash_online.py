...
#############################################################################################################
image = 0 # 0=openESI / 1=openMips
if distro.lower() == "openmips":
	image = 1
elif distro.lower() == "openesi":
	image = 0
feedurl_esi = 'http://www.openesi.eu/images/%s' %ImageVersion

if ImageVersion == '6.2':
	ImageVersion2= '6.3'
else:
	ImageVersion2= '6.2'
feedurl_esi2= 'http://www.openesi.eu/images/%s' %ImageVersion2
...
class doFlashImage(Screen):
	...
	def box(self):
		...
		if box in ('uniboxhd1', 'uniboxhd2', 'uniboxhd3'):
			...
			box = box[0:3] + 'x00'
		elif box == 'odinm9' and self.feed == "esi":
			box = 'maram9'
		return box
	...
	def checkTrafficLight(self):
		...
		try:
			urlopenESI = "http://www.openesi.eu/semaforo"
			d = urllib2.urlopen(urlopenESI)
			tmpStatus = d.read()
		except:
			tmpStatus = _("Unknown")
		...
		if self.Online:
			...
			if image == 1:
				if self.feed == "esi":
					...
					self["key_blue"].setText("openESI")
			else:
				if self.feed == "esi":
					self.feedurl = feedurl_esi
					self["key_blue"].setText("ESI %s" %ImageVersion2)
				else:
					self.feedurl = feedurl_esi2
					self["key_blue"].setText("ESI %s" %ImageVersion)
			...
			for line in lines:
				if line.find("<a href='%s/" % box) > -1:
					...
					if self.feed == "esi" or self.feed == "esi2":
