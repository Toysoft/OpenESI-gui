...
#				print "resolved to PLAY"
	...
	def pauseServiceYellow(self):
		if config.plugins.esiteam_yellowkey.list.value == '0':
			self.audioSelection()
		elif config.plugins.esiteam_yellowkey.list.value == '2':
			ToggleVideo()
		else:
...
class InfoBarExtensions:
	...
	def quickmenuStart(self):
		try:
			if not self.session.pipshown:
				from Plugins.Extensions.ESIteam.QuickMenu import QuickMenu
	...
	def getSoftcamPanel(self):
		if config.plugins.showesiteamextensions.value:
	...
	def openSoftcamPanel(self):
		from Plugins.Extensions.ESIteam.SoftcamPanel import SoftcamPanel
		self.session.open(SoftcamPanel)

	def openRestartNetwork(self):
		try:
			from Plugins.Extensions.ESIteam.RestartNetwork import RestartNetwork
...
class InfoBarESIteam:
	"""ESI-Team - handles the esiteam action"""
	def __init__(self):
		self["ESIteamActions"] = HelpableActionMap(self, "InfoBarESIteam",
			{
				"esiteam": (self.selectRedKeytask, _("ESI-Team...")),
	...
	def selectRedKeytask(self):
		...
		if isWEBBROWSER or isHBBTV:
			...
			elif config.plugins.esiteam_redpanel.selection.value == '0':
				self.instantRecord()
			else:
				self.doRedKeyTask()
	...
	def doRedKeyTask(self):
		try:
			if config.plugins.esiteam_redpanel.selection.value =='0':
				self.instantRecord()
			elif config.plugins.esiteam_redpanel.selection.value =='1':
				from Plugins.Extensions.ESIteam.plugin import ESIteam
				self.session.open(ESIteam, services = self.servicelist)
			elif config.plugins.esiteam_redpanel.selection.value == '2':
				self.session.open(TimerEditList)
			elif config.plugins.esiteam_redpanel.selection.value == '3':
				self.showMovies()
			elif config.plugins.esiteam_redpanel.selection.value == '4':
				self.StartsoftcamPanel()
			else:
				self.StartPlugin(config.plugins.esiteam_redpanel.selection.value)
	...
	def softcamPanel(self):
		try:
			if config.plugins.esiteam_redpanel.selectionLong.value =='0':
				self.instantRecord()
			elif config.plugins.esiteam_redpanel.selectionLong.value =='1':
				from Plugins.Extensions.ESIteam.plugin import ESIteam
				self.session.open(ESIteam, services = self.servicelist)
			elif config.plugins.esiteam_redpanel.selectionLong.value == '2':
				self.session.open(TimerEditList)
			elif config.plugins.esiteam_redpanel.selectionLong.value == '3':
				self.showMovies()
			elif config.plugins.esiteam_redpanel.selectionLong.value == '4':
				self.StartsoftcamPanel()
			else:
				self.StartPlugin(config.plugins.esiteam_redpanel.selectionLong.value)
	...
	def yellow_key(self):
		...
		if not self.LongButtonPressed:
			if config.plugins.esiteam_yellowkey.list.value == '0':
				...
				AUDIO = True
				ToggleVideo()
			elif config.plugins.esiteam_yellowkey.list.value == '3':
				self.startTeletext()
			else:
				try:
					self.startTimeshift()
			...
			if config.plugins.esiteam_yellowkey.listLong.value == '0':
				from Screens.AudioSelection import AudioSelection
				self.session.openWithCallback(self.audioSelected, AudioSelection, infobar=self)
			elif config.plugins.esiteam_yellowkey.listLong.value == '2':
				global AUDIO
				AUDIO = True
				ToggleVideo()
			elif config.plugins.esiteam_yellowkey.listLong.value == '3':
				self.startTeletext()
			else:
				try:
					self.startTimeshift()
